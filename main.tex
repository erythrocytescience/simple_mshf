...
\begin{equation}
	\displaystyle
	p_f(x) = A_{II} e^{\lambda x} + B_{II} e^{-\lambda x} + C_{II} x + D_{II}, \quad
	H \leq x \leq h.
...
\begin{equation}
	\displaystyle
	Q_{II} = - \delta H \left. \frac{d p_f}{d x} \right|_{x=H}
	= - \delta H \left( A_{II} \lambda e^{\lambda H} - B_{II} \lambda e^{-\lambda H} + C_{II} \right),
...
Выражение для определения $p^f_{\Gamma}$ получим из условия $Q_I = Q_{II}$:
\begin{equation}
	\displaystyle
	\begin{gathered}
		p^f_{\Gamma} = p^R_{\Gamma} - \frac{\theta G_I +  J_{II}}{ \theta F_I + K_{II}}, \\
		J_{II} = \delta H \, C_{II} \left(1 - \dfrac{2}{\eta}\right), \quad
		K_{II} =  \delta H\, \lambda \tanh\!\big(\lambda (H-h)\big). % chktex 21
	\end{gathered}
...
\begin{equation}
	...
	% \theta = \dfrac{G_I K_{II} - F_I J_{II}}{F_I \bigl(Q_{\text{num}} - Q_w\bigr)} -  \dfrac{K_{II}}{F_I}.
...
\begin{equation}
	Q_f \;=\; \frac{1}{d} \iint\limits_{\Omega} \bigl(p_d - p_f\bigr)\, dx,
	\label{eq:kham_fract_inner_Q2}
\end{equation}
где $\Omega = [0,H]\times[0,h] \setminus \gamma_f$,  
$p_d$ определяется выражением~\eqref{eq:kham_hw_inner_total_pressure},  
