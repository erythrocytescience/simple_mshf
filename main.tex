...
\begin{figure}[H]
	\centering
	\includesvg[width=0.5\linewidth]{images/hw/hw_inner/kham_hw_inner_theta_mrst.svg}
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
		\includesvg[width=\linewidth]{images/hw/hw_inner/kham_hw_inner_Eq_mrst.svg}
		\caption{}
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
		\includesvg[width=\linewidth]{images/hw/hw_inner/kham_hw_inner_Epg_mrst.svg}
		\caption{}
...
\begin{figure}[H]
	...
	% --- Первый ряд ---
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQ_L0.5_mrst.svg}
		\caption{}
		\label{fig:kham_hw_pipe_edge_effect_L0.5_mrst}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQ_L1_mrst.svg}
		\caption{}
	...
	% --- Второй ряд ---
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQ_L5_mrst.svg}
		\caption{}
		\label{fig:kham_hw_pipe_edge_effect_L5_mrst}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQ_L10_mrst.svg}
		\caption{}
...
\begin{equation}
	\displaystyle
	G \left(L, H \right) = 1 + \dfrac{f \left(H\right)}{\left(L/H\right)^m}, %chktex 3
...
\begin{figure}[H]
	...
	% --- Первый ряд ---
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQskin_L0.5_mrst.svg}
		\caption{}
		\label{fig:kham_hw_pipe_edge_effect_L0.5_r_approx}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQskin_L1_mrst.svg}
		\caption{}
	...
	% --- Второй ряд ---
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQskin_L5_mrst.svg}
		\caption{}
		\label{fig:kham_hw_pipe_edge_effect_L5_r_approx}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\textwidth}
		\includesvg[width=\linewidth]{images/hw/rad_hypo_fault/kham_EQskin_L10_mrst.svg}
		\caption{}
...
\begin{equation}
	...
	\begin{gathered}
		...
		p_{\Gamma} = \dfrac{2 H \ln{\left(H/r_w\right)}}{2 H \ln{\left(H/r_w\right)
				+ \pi \left(1 - H\right)}}, \\[6pt] % chktex 3		 
		s = \dfrac{\ln \left( H / r_w \right)\,\left(1 - G\right)}{G}, \quad
		G = 1 + \dfrac{a + b/H}{\left(L/H\right)^{m}}, \\ %chktex 3 
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
		\includesvg[width=\linewidth]{images/hw/kham_hw_models_compare_rectangle_bound_area.svg}
		\caption{}
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
		\includesvg[width=\linewidth]{images/hw/kham_hw_models_compare_square_bound_area.svg}
		\caption{}
...
\begin{tcolorbox}[colback=yellow!20, colframe=black, coltitle=black, colbacktitle=yellow, title=\textbf{Предупреждение}]
	рис.~\ref{fig:kham_fract_inner_scheme}:
	\begin{enumerate}
		\item Найти рисунок в формате SVG
		\item ДОБАВИТЬ на рисунок обозначения $\gamma_f$, $\Gamma_f$.
	\end{enumerate}
\end{tcolorbox}
...
\end{equation}
где~$\gamma_f = \{(x,z): x^2+z^2 = r_w^2,\; x\ge 0,\; z\ge 0\}$~---~это четверть окружности радиуса~$r_w$ с центром в начале координат,
...
Тогда в подобласти, ограниченной окружностями $\gamma_f$ и $\Gamma_f$ и лучами $x=0$ и $z=0$~(рис.~\ref{fig:kham_fract_inner_scheme})
...
Подставив $A_{II}$, $B_{II}$ из~\eqref{eq:kham_hw_inner_part_II_a_b_slau_solution} в~\eqref{eq:kham_fract_inner_part_II_pf_mono_solution},
а $C_{II}$, $D_{II}$ из~\eqref{eq:kham_fract_inner_part_II_cf_c_d} в~\eqref{eq:kham_fract_inner_part_II_partical_pf_view},
и с учётом~\eqref{eq:kham_common_final_pf_ode_solution_view} получим аналитическое решение уравнения~\eqref{eq:kham_hw_inner_part_II_pf_1d_equation}:
\begin{equation}
	\displaystyle
	p_f^{II}(x) = A_{II} e^{\lambda x} + B_{II} e^{-\lambda x} + C_{II} x + D_{II}, \quad
	H \leq x \leq h.
	\label{eq:kham_fract_inner_part_II_final_pf}
\end{equation}
...
\begin{equation}
	\displaystyle
	\begin{gathered}
		\left. \dfrac{d p_f}{dx} \right|_{x=H} = - \lambda \left(p_{\Gamma}^f - p_{\Gamma}^R \right)
		\tanh\left(\lambda\left(h - H \right)\right) + C_{II} \left(1- \dfrac{1}{\eta}\right), \\[6pt]
		Q_{II} = M H \left. \frac{d p_f}{d x} \right|_{x=H}
		= F_{II} \left(p_{\Gamma}^f - p_{\Gamma}^R \right) + G_{II}, \\
		F_{II} = - M H \lambda \tanh\left(\lambda\left(h - H \right)\right), \quad
		G_{II} = M H C_{II} \left(1- \dfrac{1}{\eta}\right).
	\end{gathered}
	\label{eq:kham_fract_inner_part_II_Q}
\end{equation}

\mysub{Полный приток к участку скважины с трещиной ГРП }
Давление трещине полностью будет определяться через~\eqref{eq:kham_fract_inner_part_I_final_pf} и~\eqref{eq:kham_fract_inner_part_II_final_pf}
\begin{equation}
	p =
	\begin{cases}
		\left(p^f_{\Gamma} - p^R_{\Gamma}\right) \dfrac{E_I\left(r\right)}{D_I} +
		p^R_{\Gamma} \dfrac{\ln\left(r/r_w\right)}{\ln{\left(H/r_w\right)}}, \quad  r_w \leq r \leq H, \\
		A_{II} e^{\lambda x} + B_{II} e^{-\lambda x} + C_{II} x + D_{II}, \quad H \leq x \leq h.
	\end{cases}
	\label{eq:kham_fract_inner_final_press}
\end{equation}

Из условия равенства дебитов $Q_I = Q_{II}$ на границе~$\Gamma_f$ получаем:
\begin{equation}
	\displaystyle
	\begin{gathered}
		p^f_{\Gamma} = p^R_{\Gamma} + \frac{G_{II} - \theta G_I}{ \theta F_I\left(H\right) - F_{II}}.
	\end{gathered}
	\label{eq:kham_fract_inner_pg}
\end{equation}
...
\end{equation}
где $p_d$ определяется выражением~\eqref{eq:kham_hw_inner_total_pressure},
...
Зафиксируем следующие параметры: $r_w = 0.001$, $H=0.1$, $d=1$, $h=0.5$.
...
\begin{figure}[H]
	\centering
	\includesvg[width=0.6\linewidth]{images/fract/inner/assymptots/kham_fract_inner_pfg_asympt.svg}
...
\begin{figure}[H]
	\centering
	\includesvg[width=0.6\linewidth]{images/fract/inner/assymptots/kham_fract_inner_pf_asympt.svg}
...
\begin{equation}
	Q_f \to Q_f^{\max} \;=\; \dfrac{H}{2d} \int_{r_w}^H p_I(r)r dr + \dfrac{H}{d} \int_{H}^h p_{II}(x) dx,
	\label{eq:kham_fract_inner_Qf_max_asympt}
\end{equation}
где $p_I$, $p_{II}$ задаются выражением~\eqref{eq:kham_hw_inner_total_pressure}.

\begin{figure}[H]
	\centering
	\includesvg[width=0.6\linewidth]{images/fract/inner/assymptots/kham_fract_inner_qf_asympt.svg}
	\caption{Дебит $Q_f$ трещины при различных $M$}
	\label{fig:kham_fract_inner_qf_assympt}
\end{figure}

На рис.~\ref{fig:kham_fract_inner_qf_assympt}: $Q_f^{\min}=1.67\cdot10^{-5}$, $Q_f^{\max}=0.0183$.
...
\begin{description}
	\item{$\beta$}~---~коэффициент...;
	\item[$\delta$]~---~полуширина (апертура) трещины;
	\item[$\gamma_f$]~---~поверхность скважины (четверть окружности радиуса~$r_w$ с центром в начале координат);
